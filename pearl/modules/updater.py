"""
This module is used for updating BlackPearl
"""
...
from pearl import CMD_HNDLR
from pearl.utils import admin_cmd
...
UPSTREAM_REPO_URL = "https://github.com/IntelligentSammy/BlackPearl"
...
@pearl.on(admin_cmd(pattern="update ?(.*)"))
async def upstream(ups):
    ...
    try:
        ...
        if conf != "now":
            ...
            \nBut we can fix that by force updating the userbot using** `{rider}update now`."
            )
            return
        repo = Repo.init()
    ...
    if ac_br != "master":
        await ups.edit(
            ...
            "Please checkout the official branch of BlackPearl`"
        )
        repo.__del__()
        return
    ...
    if conf != "now" and not force_updateme:
        ...
        await ups.respond(f"Do `{rider}update now` to update")
        return
